    ...
    public function index()
{
    ...
    $candidate_online_offline = $CI->Candidate_model->count_candidate_online_offline();
    ...
    $short_list_oo = $CI->Reports->report_online_offline_gender(3);
    ...
    $interview_list_oo = $CI->Reports->report_online_offline_gender(4);
    ...
    $hired_list_oo = $CI->Reports->report_online_offline_gender(5);
    ...
    $rejected_list_oo = $CI->Reports->report_online_offline_gender(6);
    //report
    ...
    'total_short_list_male'=>$total_short_list_gender['Male'],
    'total_short_list_female'=>$total_short_list_gender['Female'],
    'short_list_online_male'=>$short_list_oo['online']['Male'],
    'short_list_online_female'=>$short_list_oo['online']['Female'],
    'short_list_offline_male'=>$short_list_oo['offline']['Male'],
    'short_list_offline_female'=>$short_list_oo['offline']['Female'],
    'total_interview_list'=>$total_interview_list,
    'total_interview_list_male'=>$total_interview_list_gender['Male'],
    'total_interview_list_female'=>$total_interview_list_gender['Female'],
    'interview_list_online_male'=>$interview_list_oo['online']['Male'],
    'interview_list_online_female'=>$interview_list_oo['online']['Female'],
    'interview_list_offline_male'=>$interview_list_oo['offline']['Male'],
    'interview_list_offline_female'=>$interview_list_oo['offline']['Female'],
    'total_hired_list'=>$total_hired_list,
    'total_hired_list_male'=>$total_hired_list_gender['Male'],
    'total_hired_list_female'=>$total_hired_list_gender['Female'],
    'hired_list_online_male'=>$hired_list_oo['online']['Male'],
    'hired_list_online_female'=>$hired_list_oo['online']['Female'],
    'hired_list_offline_male'=>$hired_list_oo['offline']['Male'],
    'hired_list_offline_female'=>$hired_list_oo['offline']['Female'],
    'total_rejected_list'=>$total_rejected_list,
    'total_rejected_list_male'=>$total_rejected_list_gender['Male'],
    'total_rejected_list_female'=>$total_rejected_list_gender['Female'],
    'rejected_list_online_male'=>$rejected_list_oo['online']['Male'],
    'rejected_list_online_female'=>$rejected_list_oo['online']['Female'],
    'rejected_list_offline_male'=>$rejected_list_oo['offline']['Male'],
    'rejected_list_offline_female'=>$rejected_list_oo['offline']['Female'],
        'total_sales'           => $total_sales,
        'total_candidate'       => $total_candidate,
        'total_candidate_male'  => $total_candidate_gender['Male'],
        'total_candidate_female'=> $total_candidate_gender['Female'],
        'online_male'           => $candidate_online_offline['online']['Male'],
        'online_female'         => $candidate_online_offline['online']['Female'],
        'offline_male'          => $candidate_online_offline['offline']['Male'],
        'offline_female'        => $candidate_online_offline['offline']['Female'],
...
// product wise profit report form
    ...
    // Delete a report entry for a candidate (match by seeker_id → candidates.id, then candidate_report)
    // Supported types: shortlisted=3, interviewed=4, hired=5, rejected=6
    public function delete_report($seeker_id = null, $type = null) {
        $CI =& get_instance();
        $this->auth->check_admin_auth();
        $CI->load->model('Reports');
        $CI->load->model('Candidate_model');

        if ($seeker_id === null) { show_404(); return; }

        // Step 1 – select id from candidates based on seeker_id
        $candidate = $CI->db->select('id, full_name')
                            ->from('candidates')
                            ->where('seeker_id', $seeker_id)
                            ->get()
                            ->row_array();
        if (empty($candidate)) { show_404(); return; }

        $candidate_id = $candidate['id'];

        // Step 2 – delete from candidate_report where candidate_id=id and status matches type
        $statusMap = [
            'shortlisted' => 3,
            'interviewed' => 4,
            'hired'       => 5,
        ...
        $status = isset($statusMap[$type]) ? $statusMap[$type] : 5; // default to hired
        $CI->db->where('candidate_id', $candidate_id)
               ->where('status', $status)
               ->delete('candidate_report');

        // Step 3 – set candidates.status back to job-seeker (status=1)
        $CI->db->where('id', $candidate_id)
               ->update('candidates', [
                   'status'          => 1,   // job seeker / available again
                   'profile_complete'=> 0,
                   'assigned_to'     => NULL,
               ]);
        ...
        $redirect = $type && isset($statusMap[$type])
            ? base_url("Admin_dashboard/{$type}_by_company/0")  // company_id 0 = overview list
            : base_url('Admin_dashboard/hired_by_company/0');
        redirect($redirect);
    }
